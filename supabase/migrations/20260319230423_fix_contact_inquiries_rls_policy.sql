/*
  # Fix contact_inquiries RLS policy

  1. Security Changes
    - Drop the overly permissive "Allow anonymous inserts for contact form" policy
    - Replace with a restrictive policy that validates required fields are non-empty
    - This ensures only properly filled-out form submissions are accepted

  2. Important Notes
    - The new policy checks that full_name, phone, and email are not empty strings
    - This prevents spam/empty submissions at the database level
*/

DROP POLICY IF EXISTS "Allow anonymous inserts for contact form" ON contact_inquiries;

CREATE POLICY "Anon can insert with valid required fields"
  ON contact_inquiries
  FOR INSERT
  TO anon
  WITH CHECK (
    full_name IS NOT NULL AND full_name <> '' AND
    phone IS NOT NULL AND phone <> '' AND
    email IS NOT NULL AND email <> ''
  );
